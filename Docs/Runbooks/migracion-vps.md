...
5. **Activación operativa por Manu:** el workflow escribe `BOT_NATIVE_PROTECTION_ENABLED: "false"` en `/docker/botclo/docker-compose.release.yml`; ese valor tiene precedencia sobre `.env`. Para activar, cambiar ese valor a `"true"` y recrear únicamente `web` usando ambos archivos: `docker compose -f docker-compose.vps.yml -f docker-compose.release.yml up -d --no-deps web`. Verificar el valor efectivo dentro del contenedor. Los próximos ticks concilian el historial y solicitan stops reales cuando la estrategia los requiere. Esto coloca órdenes en Binance; revisar cada robot y la pestaña de órdenes abiertas de Binance. El workflow actual vuelve a escribir `false` en cada deploy: después de la primera adopción, preparar los siguientes despliegues para conservar la conciliación y protección activas.
